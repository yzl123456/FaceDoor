 ...
 */
@Repository("ownerVerifyDao")
public class OwnerVerifyDaoImpl implements OwnerVerifyDao
{
    @Autowired
	...
	public TableData getOwnerVerifyList(TableFilter tableFilter) {
        ...
        {
            String hql = "from OwnerVerifyEntity d where d.name like (?)";
            query = session.createQuery(hql).setString(0, "%" + tableFilter.getSearchValue() + "%");
        }
        else
        {
            String hql = "from OwnerVerifyEntity d";
        ...
        tableData.setRecordsTotal(this.count("OwnerVerifyEntity"));
